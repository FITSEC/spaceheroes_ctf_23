...
def print_top(guess, uuid):
    # Spawn our background process
    cat_check = 0
    if(guess == True):
        setproctitle.setproctitle(str(uuid))
        setproctitle.setthreadtitle(str(uuid))
        #cat_check = subprocess.Popen(["bash", "-c", "exec", "-a", f"{str(uuid)}", "less", "checker.txt"], preexec_fn=os.setsid, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        #command = f"exec -a {str(uuid)} less checker.txt"
        #print(command)
        #cat_check = subprocess.Popen(command, shell=True, preexec_fn=os.setsid, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)


    # Print Header & CPU Usage Bars
    ...
    # Iterate over all running processes and make dict
    listOfProcessNames = []
    ...
    # Print header and all processes
    ...
    # Kill off background process
    if (guess == True):
        setproctitle.setproctitle("python3")
        setproctitle.setthreadtitle("python3")
        """
        cat_check.communicate()
        cat_check.kill()
        try:
            os.killpg(os.getpgid(cat_check.pid), signal.SIGTERM)
        except:
            cat_check = 0
        """
...
def main():
    ...
    for i in range(0, len(flag)):
        if i+1 > len(user_guess): # GET RID OF THIS TO REMOVE FINAL CHECK
    ...
    # Print flag if user input is correct
    if user_guess == flag:
